...
while true; do
for box in ${RACK[@]}; do $cmd $box "$var list" 2>/dev/null|$logger; done; sleep 15
for box in ${RACK[@]}; do $cmd $box "$var sel" 2>/dev/null|$logger; done; sleep 10
for box in ${RACK[@]}; do $cmd $box "$var sel" 2>/dev/null|$logger; done; sleep 10
for box in ${RACK[@]}; do $cmd $box "$var sel" 2>/dev/null|$logger; done; sleep 10
for box in ${RACK[@]}; do $cmd $box "$var stop" 2>/dev/null|$logger; done; sleep 15
for box in ${RACK[@]}; do $cmd $box "$var e" 2>/dev/null|$logger; done; sleep 15
