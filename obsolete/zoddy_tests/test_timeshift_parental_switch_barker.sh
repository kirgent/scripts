...
while true; do
echo "iteration=$i"
for ip in ${RACK[@]}; do $cmd $ip "$var rew"; done; check_health; sleep 10
for ip in ${RACK[@]}; do $cmd $ip "pwreg set block_times 00"; done; check_health; sleep 15
for ip in ${RACK[@]}; do $cmd $ip "$var pause"; done; check_health; sleep 10
for ip in ${RACK[@]}; do $cmd $ip "pwreg set block_times"; done; check_health; sleep 15
